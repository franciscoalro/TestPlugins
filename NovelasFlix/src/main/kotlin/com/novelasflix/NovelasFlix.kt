...
/**
 * NovelasFlix - Provider focado em novelas
 * 
 * Suporte a:
 * - Novelas mexicanas
 * - Novelas brasileiras
...
class NovelasFlix : MainAPI() {
    ...
    companion object {
        private const val TAG = "NovelasFlix"
    }
    ...
    private fun Element.toSearchResult(): SearchResponse? {
        return try {
            val title = select("h3.poster__title a span").text().trim()
            val href = select("h3.poster__title a").attr("href")
            val posterUrl = select("div.poster__img img").attr("src")
            val yearText = select("div.bslide__meta span").firstOrNull()?.text()?.trim()
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        return try {
            ...
            val results = document.select("div#dle-content div.default.poster.grid-item.has-overlay")
                .mapNotNull { it.toSearchResult() }
    ...
    override suspend fun loadLinks(
        ...
        return NovelasFlixExtractor.extractVideoLinks(data, mainUrl, name, subtitleCallback, callback)
    }
